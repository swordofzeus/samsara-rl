...
def test_post_episode_updates_q_table(gw_mdp, random_policy):
    """post_episode should update Q-table entries for visited (S, A) pairs."""
    mc = MonteCarloPrediction(gw_mdp, random_policy, gamma=0.9)
    trajectory = np.array([[10.0, 1.0, -1.0], [14.0, 2.0, -1.0]])
    mc.post_episode(trajectory)
    assert mc.q_table[10, 1] != 0, "Q(10, 1) should be updated"
    assert mc.q_table[14, 2] != 0, "Q(14, 2) should be updated"


def test_post_episode_does_not_update_unvisited(gw_mdp, random_policy):
    """post_episode should not modify Q values for unvisited (S, A) pairs."""
    mc = MonteCarloPrediction(gw_mdp, random_policy, gamma=0.9)
    trajectory = np.array([[10.0, 1.0, -1.0], [14.0, 2.0, -1.0]])
    mc.post_episode(trajectory)
    assert mc.q_table[5, 0] == 0, "Unvisited Q(5, 0) should remain 0"
    assert mc.q_table[10, 0] == 0, "Unvisited action Q(10, 0) should remain 0"


def test_post_episode_handles_duplicate_visits(gw_mdp, random_policy):
    ...
    mc.post_episode(trajectory)
    q_with_dupes = mc.q_table[10, 1]

    mc2 = MonteCarloPrediction(gw_mdp, random_policy, gamma=0.9)
...
def test_discounted_returns_no_discount(gw_mdp, random_policy):
    ...
    mc = MonteCarloPrediction(gw_mdp, random_policy, gamma=1)
    trajectory = np.array([
        [10.0, 1.0, -1.0],
        [14.0, 2.0, -1.0],
        [13.0, 3.0, -1.0],
...
def test_evaluate_convergence(gw_mdp, random_policy, expected_v_random_policy):
    ...
    mc = MonteCarloPrediction(gw_mdp, random_policy, alpha=0.01, gamma=0.9)
    mc.evaluate(max_iter=40000)
